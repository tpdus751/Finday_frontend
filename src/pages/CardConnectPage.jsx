...
export default function CardConnectPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/card/not_connected', {
          params: { userNo }
        });
        setAvailableBanks(res.data);
      } catch (err) {
        console.error('연동 가능한 카드 목록 조회 실패:', err);
      }
    };
    fetchData();
  }, [userNo]);
  ...
  const handleNext = () => {
    ...
    navigate('/cards/connect/consent', {
      state: {
        selectedBanks,
        userSpecificNo
      }
  ...
  return (
    <PageWrapper>
      <Card>
        <Title>연동할 카드 금융사 선택</Title>
        {availableBanks.length === 0 ? (
          <Notice>연동 가능한 카드 금융사가 없습니다.</Notice>
        ) : (
          <>
            <BankGrid>
